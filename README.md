...
The Map type renders a rotatable Three.js globe with SVG country boundaries and location-entity nodes. A Moon sized proportionally to Earth follows a time-compressed orbit at its mean physical distance while keeping its near side tidally locked toward Earth. Its flat paper-and-ink treatment matches the map, and dragging rotates the complete Earth–Moon system. The default camera keeps Earth at map scale, so the distant Moon is normally off-canvas until the view is zoomed out. Node position comes only from the reviewed `data/location_coordinates.json` gazetteer; ambiguous and unmapped names are reported but never guessed onto the globe. Map nodes retain the same collection, confidence, prominence-inflation, size, label, and evidence-inspection controls as other entity views.
