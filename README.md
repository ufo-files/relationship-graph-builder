...
Every graph type exports the same presentation-ready PDF format. Its cover identifies UFO Files, the graph title and type, export time, catalog generation time, machine-data repository and revision, and links to `https://ufo-files.app` and `https://github.com/ufo-files`. The following page captures the complete presentation stage—title, subtitle, graph or table, visible relationship layer, legend, result summary, and evidence-policy detail—without the interactive fullscreen, animation, reset, or export controls.
