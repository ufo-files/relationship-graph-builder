...
The Book type is a packed area view of upright 2:3 book-cover shapes snapped to integer dimensions. Each block is a title explicitly identified in transcript text, with area proportional to its raw mention count and shade controlled independently by the selected prominence metric. Smaller books fill open space around larger ones, and readable covers use centered serif title typography. Titles are extracted only from book, novel, or memoir cues; each mark opens its source evidence in the inspector.
