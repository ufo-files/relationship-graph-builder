...
const scatterPresetPrefix = "scatter-preset-";
let scatterPresets = [];
...
function galleryMarkup(graphTypes) {
  ...
  const presetRows = [];
  for (let index = 0; index < scatterPresets.length; index += 2) {
    const cells = scatterPresets.slice(index, index + 2).map(({ id, label }) => `    <td width="50%" align="center">
      <strong>${escapeHTML(label)}</strong><br>
      <a href="assets/screenshots/${scatterPresetPrefix}${escapeHTML(id)}.png"><img src="assets/screenshots/${scatterPresetPrefix}${escapeHTML(id)}.png" alt="${escapeHTML(label)} Scatter preset screenshot" width="100%"></a>
    </td>`);
    if (cells.length === 1) cells.push('    <td width="50%"></td>');
    presetRows.push(`  <tr>\n${cells.join("\n")}\n  </tr>`);
  }
...
try {
  ...
  scatterPresets = await page.locator("[data-preset-select] option:not([value=''])").evaluateAll(options => options.map(option => ({
    id: option.value,
    label: option.textContent.trim()
  })));
  if (!scatterPresets.length) throw new Error("No Scatter view presets were found");
  if (scatterPresets.some(({ id }) => !/^[a-z0-9-]+$/.test(id))) {
    throw new Error("Preset IDs must contain only lowercase letters, numbers, and hyphens");
  }
  ...
  const expectedFiles = new Set([
    ...
    ...scatterPresets.map(({ id }) => `${scatterPresetPrefix}${id}.png`)
  ]);
  ...
  for (const { id } of graphTypes) {
    ...
    if (id === "solar") {
      await page.waitForFunction(() => window.ufoSolar?.isReady());
    }
  ...
  await page.locator('[data-type="scatter"]').click();
  for (const { id } of scatterPresets) {
    await page.locator("[data-preset-select]").selectOption(id);
    await page.waitForFunction(presetId => document.querySelector("[data-preset-select]")?.value === presetId, id);
    await page.waitForFunction(() => !document.querySelector("#toast")?.classList.contains("show"));
    await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
    await page.locator(".stage").screenshot({
      animations: "disabled",
      path: path.join(outputDirectory, `${scatterPresetPrefix}${id}.png`),
      type: "png"
    });
  }
