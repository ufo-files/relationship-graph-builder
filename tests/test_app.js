...
test("Galactic Entities boots from a compact astronomy payload", () => {
  ...
  assert.match(fs.readFileSync("map-globe.js", "utf8"), /three\.module\.min\.js\?v=astronomy-renderer-v2/);
  assert.match(fs.readFileSync("map-globe.js", "utf8"), /SVGLoader\.js\?v=astronomy-renderer-v2/);
  assert.match(fs.readFileSync("vendor/addons/SVGLoader.js", "utf8"), /three\.module\.min\.js\?v=astronomy-renderer-v2/);
  const threeModule = fs.readFileSync("vendor/three.module.min.js", "utf8");
  assert.equal(threeModule.match(/three\.core\.min\.js\?v=astronomy-renderer-v2/g)?.length, 2);
...
test("Hill–Fish is an explicit claim layer over current measured star positions", () => {
  ...
  assert.doesNotMatch(moduleSource, /[{,]\s*\d[\w$]*\s*:/, "object keys beginning with digits must be quoted for Firefox");
